#ifndef STACKEDMARKERGRAPH_H
#define STACKEDMARKERGRAPH_H
...
class StackedMarkerGraph : public QWidget
{
    Q_OBJECT
public:
    explicit StackedMarkerGraph(QWidget *parent = nullptr) : QWidget{parent}
    {
        QVector<int> a;
        for (int i = 0; i < 5; ++i)
...
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            painter->drawEllipse(mp[0][i].first * dx - 4, -mp[0][i].second * dy - 4, 8, 8);
            painter->drawEllipse(mp[0][i].first * dx - 4, -(mp[0][i].second + mp[1][i].second) * dy - 4, 8, 8);
            painter->drawEllipse(mp[0][i].first * dx - 4, -(mp[0][i].second + mp[1][i].second + mp[2][i].second) * dy - 4, 8, 8);
