#ifndef SLICEDRINGDIAG_H
#define SLICEDRINGDIAG_H
...
class SlicedRingDiag : public QWidget
{
    Q_OBJECT
public:
    explicit SlicedRingDiag(QWidget *parent = nullptr) : QWidget{parent}
    {
        for (int i = 0; i < 5; ++i)
...
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            painter->setPen(Qt::white); // скорее всего нужно будет palette().color(QPalette::Window)
            painter->setBrush(Qt::white);
            painter->drawPie(QRect(-r, -r, 2 * r, 2 * r), tmp * 12, -tmp * 24);
            painter->setPen(Qt::black);
            painter->drawArc(QRect(-r, -r, 2 * r, 2 * r), tmp * 8, -tmp * 16);
            angle += tmp / 2;
            painter->save();
