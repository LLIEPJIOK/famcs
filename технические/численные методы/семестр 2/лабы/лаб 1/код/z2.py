...
def chebyshev_nodes(a: float, b: float, n: int) -> list[float]:
    """Генерирует n+1 узлов Чебышёва на отрезке [a, b]."""
    return [math.cos((2 * k + 1) * math.pi / (2 * (n + 1))) * (b - a) / 2 + (b + a) / 2 for k in range(n + 1)]
...
for idx, n in enumerate(degrees):
    num_nodes: int = n + 1
    x_nodes: list[float] = chebyshev_nodes(-2, 2, n)
